...
io.on('connection', function (socket) {
  ...
  socket.on('newroomname', function (data) {
    ...
    roomarr.push(data);
    io.emit('viewroom', roomarr, roomarr[0]);
  })
  ...
  socket.on('add user', function (username) {
    ...
    socket.emit('viewroom', roomarr, roomarr[0]);
  ...
  socket.on('switchRoom', function (newroom) {
    socket.leave(socket.room);
    numUsers--;
    socket.broadcast.to(socket.room).emit('userleaved', {
      username: socket.username,
      numUsers: numUsers
    });
    socket.join(newroom);
    ...
    numUsers++
    socket.room = newroom;
    socket.broadcast.to(socket.room).emit('joined user', {
      username: socket.username,
      numUsers: numUsers
    });
    console.log("user joined" + socket.room);
    socket.emit('viewroom', roomarr, newroom);
  })
});
